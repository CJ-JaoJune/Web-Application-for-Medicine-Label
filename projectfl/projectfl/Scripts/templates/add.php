<?php 
    ...
    if (isset($_REQUEST['btn_insert'])) {
        try {
            ...
            $temp = $_FILES['txt_file']['tmp_name']; //floder ปลายทาง

            $path = "upload/" . $newname; // set upload folder path
            ...
            if (empty($name)) {
                ...
                if (!file_exists($path)) { // check file not exist in your upload folder path
                    if ($size < 5000000) { // check file size 5MB
                        move_uploaded_file($temp, 'upload/'.$newname); // move upload file temperory directory to your upload folder
                    } else {
                ...
                $errorMsg = "Upload JPG file formate...";
            }

            if (!isset($errorMsg)) {
                $insert_stmt = $db->prepare('INSERT INTO tbl_file(name, image) VALUES (:fname, :fimage)');
                $insert_stmt->bindParam(':fname', $name);
                $insert_stmt->bindParam(':fimage', $newname);

                /*$result = "INSERT INTO tbl_date(
                                                date_now,
                                                name_img, 
                                                image_file) 
                                        VALUES (
